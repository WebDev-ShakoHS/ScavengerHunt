<?php 
	...
	$db = mysqli_connect('localhost', 'root', '', 'scavenger');
	...
	//	$email = mysqli_real_escape_string($db, $_POST['email']);
		...
		if (empty($members)) { array_push($errors, "Team members are required"); }
	//	if (empty($email)) { array_push($errors, "Email is required"); }
		...
		if (count($errors) == 0) {
			...
			$query = "INSERT INTO users (username, members, password) 
					  VALUES('$username', '$members', '$password')";
